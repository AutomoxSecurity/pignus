...
Testing
Unit Tests at automox/pignus/tests/unit/utils/test_misc_server.py
...
def get_option_value(option_name: str, default=None):
    ...
    option = settings.options.get(option_name)
    if not option:
        return default
    return option.value
...
def get_pignus_migrations_path() -> str:
    ...
    pignus_path = os.environ.get("PIGNUS_MIGRATIONS_PATH")
    if not pignus_path:
        log.error(
...
def set_keys() -> bool:
    """Collect the RSA key pair from SSM and set it in the global settings var to be used through
    ...
    settings.server["KEYS"]["PUBLIC"] = public_key
    if not isinstance(settings.server["KEYS"]["PUBLIC"], rsa.key.PublicKey):
        log.warning("Invalid public key: %s" % settings.server["KEYS"]["PUBLIC"])
    ...
    settings.server["KEYS"]["PRIVATE"] = private_key
    if not isinstance(settings.server["KEYS"]["PRIVATE"], rsa.key.PrivateKey):
        log.warning("Invalid private key: %s" % settings.server["KEYS"]["PRIVATE"])
...
# End File: automox/pignus/src/pignus/utils/misc_server.py
