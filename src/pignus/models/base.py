...
Testing
Unit Tests: automox/pignus/tests/unit/models/test_base_model.py
23/30 Tested
...
class Base:
    ...
    def _establish_db(self, conn, cursor) -> bool:
        self.conn = conn
        if not self.conn and "conn" in settings.db:
            self.conn = settings.db["conn"]
        self.cursor = cursor
        if not self.cursor and "cursor" in settings.db:
            self.cursor = settings.db["cursor"]
        return True

# End File: automox/pignus/src/pignus/models/base.py
